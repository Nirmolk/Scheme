...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        rest_operators= rest.map(lambda x: scheme_eval(x,env))
        return scheme_apply(operator, rest_operators,env)
...
def eval_all(expressions, env, tail=False):
    ...
    if expressions is nil:
        ...
        return eval_all(expressions.second, env, tail)
    # END PROBLEM 8
...
def do_lambda_form(expressions, env):
    ...
    lambda_inst= LambdaProcedure(formals,body,env)
    return lambda_inst
    "*** YOUR CODE HERE ***"
    # END PROBLEM 9
...
def do_and_form(expressions, env):
    ...
    check = scheme_eval(expressions.first,env)
    
    if scheme_falsep(check):
        return False
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        """
        if tail and not scheme_symbolp(expr):
