...
;;; ********************************
; BEGIN PROBLEM 0
(define (rip_stan_lee age)
  (if (> age 94) '(RIP) '(Long live the real hero)))
; expect rip_stan_lee
...
(rip_stan_lee 94)
; expect (long live the real hero)
...
(define cake (eval (+ apple 30)))
; expect cake

(- apple cake)
; expect -30

(define orange (+ 1 orange))
; expect Error
...
(define (divide y) ( / 40 x))
;expect divide

divide
; expect (lambda (y) (/ 40 x))
...
(define hellohey (lambda (x y) (hello (+ x x) y)))
; expect hellohey
(hellohey 8 7)
; expect 23
